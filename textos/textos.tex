...
como \code{NaN} (\emph{not-a-number}), infinito o indefinido, con las funciones
\code{isnan}, \code{isfinite} e \code{isdefined}, respectivamente;
...
hay definida una variable con el símbolo ``x'' (\code{:x} con los dos puntos
est una forma abreviada de \code{symbol("x")}), y si se cumple esa condición,
...
están pensados para componer \emph{arrays} de tipo \code{Bool}, no valores
individuales. Por ejemplo, supongamos que tenemos dos vectores de la misma
longitud: \code{v1}, \code{v2}, y queremos verificar qué elementos de ambos
vectores tienen valores estrictamente positivos. Esto se haría así
(nótese el punto en el operador de comparación para aplicarlo a \code{arrays}):
...
También se podría usar en un bucle \code{for} aplicado a esos dos elementos, con la
función auxiliar \code{find} que devuelve los índices positivos para un
vector de \code{Bool}:
...
no usarlos para evaluar condiciones tipo \code{if} o bucles \code{while}.
