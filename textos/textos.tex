...
Una característica interesante de Julia es que permite concatenar estos operadores lógicos como se hace a menudo al escribir relaciones matemáticas. Por ejemplo, para verificar si un valor de probabilidad \code{p} está efectivamente acotado entre \code{0} y \code{1}:
...
Los vectores de valores lógicos como este resultado no se pueden usar como condición en un \code{if} o \code{while}, que sólo admiten valores de tipo \code{Bool} unitarios. Uno de los propósitos con los que se suelen usar es para ``seleccionar'' los elementos de otro vector sobre los que se quiere realizar alguna operación. Por ejemplo, supongamos que queremos calcular el logaritmo del vector \code{v1}, dejando como indefinidos (\code{NaN}) aquellos elementos cuyo valor en \code{v1} no es estrictamente positivo.
...
También es útil mencionar que por convención los valores lógicos se pueden interpretar numéricamente, considerando \code{false==0} y \code{true==1}. Así pues, se puede comprobar cuántos valores positivos hay en el vector \code{v1} mediante:

julia> sum(positivos)
3

En el sentido inverso, se considera que el cero es equivalente a \code{false}, y cualquier otro número (positivo o negativo) equivale a \code{true}. Esta conversión se hace con la función \code{bool}:
...
Pero es importante señalar que esta conversión no es automática, y una operación pensada para valores lógicos puede dar errores o valores inesperados si se aplica a valores numéricos. Esto se puede comprobar, por ejemplo, aplicando la función \code{any} al vector numérico \code{v1}.

Volviendo a los bloques \code{if} y \code{while}, lo que sí se puede hacer con los vectores lógicos es, por ejemplo, obtener un valor lógico unitario en función de si todos o alguno de sus elementos es verdadero:
...
\begin{description}
  ...
  Esta función se puede usar de distintas maneras. Si \code{x} es un \emph{array} u otro tipo de variable compuesta por varios componentes, se interroga si esa variable contiene el componente representado por el índice \code{index}; p.ej. \code{isdefined(v1, 5)}. También se puede usar con un solo argumento, consistente en el nombre de una variable en forma de ``símbolo'' ---la forma más sencilla de hacer esto es añadiendo dos puntos delante del nombre---; en ese caso se interroga si existe una variable con ese nombre en el espacio de trabajo, p.ej. \code{isdefined(:v2)}.
...
En todos los ejemplos de bucles \code{for} mostrados hasta este punto
las iteraciones se han hecho sobre rangos como \code{1:10}, etc.
...
bucles (con el símbolo ``\code{=}'' o con ``\code{in}'', véase arriba),
...
en los bucles \code{for} son los \emph{arrays} y los ``diccionarios''.
